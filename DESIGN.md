...
- **Hero data stream:** decorative falling glyphs and packets, capped at 30fps with a 1.5× device-pixel-ratio ceiling. Stops when offscreen, when the tab is hidden. Reduced motion paints a still frame and hides the unnecessary pause control. The canvas and artwork are hidden from assistive technology.
